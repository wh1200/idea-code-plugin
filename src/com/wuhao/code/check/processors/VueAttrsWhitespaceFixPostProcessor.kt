...
import org.jetbrains.vuejs.cli.VueProjectTemplateFactory
              ...
              if (element.attributes.any { it.name == "slot" } && version == 3) {
                ...
                val text = if (tagName == TEMPLATE_TAG) {
                  ...
                  """<template $slotExp>
                    <${tagName} ${
                    element.attributes.filter { it.name !in listOf("slot", "slot-scope") }.joinToString(" ") { it.text }
                  }>${element.value.text}</${tagName}></template>"""
                }
                val newTag = factory.createTagFromText(text, VueLanguage.INSTANCE)
