...
public class EntityCacheManager {
  ...
  private static Entity parseEntity(Class<?> entityClass) {
    ...
    entity.setBeanClass(entityClass);
    ...
    for (Field field : BeanUtil.findOrderedFields(entityClass)) {
      ...
      if (isVersion) {
        entity.setVersionField(entityField);
        versionCount++;
        if (versionCount > 1) {
    ...
    entity.setNonePrimaryKeyFieldMap(entityFieldMap);
    entity.setNonePrimaryKeyOrderedFields(orderedFields);
    entity.setNonePrimaryKeyColumnMap(entityColumnMap);
